...
INTERMEDIATE_GRADES = "Quiz1Intermediate.csv"
FINAL_GRADES = "Quiz1Full.csv"
...
def setup_autograded():
    ...
    with open(os.path.join(QUIZ_1_PATH, INTERMEDIATE_GRADES), "w", newline="") as f:
...
def create_grade_reports():
    with open(os.path.join(QUIZ_1_PATH, FINAL_GRADES)) as f:
        reader = csv.DictReader(f)

        for student in reader:
            output = [
                f"Quiz 1 Grades for {student["sid"]}",
                "",
            ] + [
                f"{q}. {student[str(q)]}" for q in QUESTIONS
            ] + [
                "",
                f"Total: {sum(float(student[str(q)]) for q in QUESTIONS):.4f}/23"
            ]

            with open(os.path.join(QUIZ_1_PATH, "Grade Reports", f"{student["sid"]}.txt"), "w") as f2:
                f2.writelines(f"{l}\n" for l in output)
